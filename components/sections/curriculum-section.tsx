    ...
    <Section id="curriculum">
      ...
      <StaggerContainer className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        {curriculum.slice(0, 8).map((course) => {
          ...
          const hasImage = !!course.image; // Check if an image URL is defined in the object properties
            ...
            >
              <Link href={`/curriculum#${course.id}`}>
                <div className="h-full p-6 bg-card rounded-2xl border border-border hover:border-primary/30 hover:shadow-lg transition-all duration-300 flex flex-col justify-between">
                  <div>
                    {/* MEDIA CONTAINER: Renders Image if available, otherwise scales the matching Icon fallback */}
